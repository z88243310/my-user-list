...
//   const pageActive = document.querySelector('.page-item.active')
//   const pageLinks = paginator.querySelectorAll('.page-item')
//   if (pageActive !== null) pageActive.classList.remove('active')
...
dataPanel.addEventListener('click', function onPanelClicked(event) {
  ...
  if (target.matches('#user-image')) {
    showUserModal(id)
    // 加入最愛
  } else if (target.matches('.far.fa-heart')) {
...
paginator.addEventListener('click', function onPaginatorClicked(event) {
  ...
  if (target.tagName === 'A') {
    renderPaginator(filterUsers.length !== 0 ? filterUsers.length : users.length)
    renderUserList(getUsersByPage(pageCurrent))
  }
})
